...
<xsl:stylesheet version="2.0"
  ...
  <xsl:import href="../../kiln/stylesheets/tei/to-html.xsl" />
     ...
     <xsl:template match="tei:add">
          <xsl:if test="@hand!='LH'"><span class="add"><xsl:apply-templates/></span></xsl:if>
     </xsl:template>
     
     <xsl:template match="tei:c[@rend='R']">
          <span class="redletter"><xsl:apply-templates/></span>
     </xsl:template>
     
     <xsl:template match="tei:cb"><xsl:choose><xsl:when test="@n!='a'">
          <span class="cb">/<xsl:value-of select="@n"/>/</span></xsl:when></xsl:choose>
     </xsl:template>
     ...
     <xsl:template match="tei:pb">
          <br/>
          <span class="pb">//<xsl:value-of select="@n"/><xsl:if test="following-sibling::tei:cb[1][@n='a']"><xsl:text>/</xsl:text><xsl:value-of select="following-sibling::tei:cb[1]/@n"/></xsl:if>//</span>
          <br/>
     </xsl:template>
     ...
     <xsl:template match="tei:q">
          <span class="quoted"><xsl:apply-templates/></span>
     </xsl:template>
     
     <xsl:template match="tei:seg[@type='ms_name']">
          <span class="ms-name"><xsl:apply-templates/><xsl:text> </xsl:text></span>
     </xsl:template>
     ...
     <xsl:template match="tei:seg[@type='1']">
          <xsl:choose>
               <xsl:when test="(starts-with(@xml:id, 'edfr20125')) and (not(@rend='NR'))">
                    <span class="seg1rfl"><xsl:apply-templates/></span>
               </xsl:when>
               <xsl:otherwise>
                    <span class="seg1"><xsl:apply-templates/></span>
               </xsl:otherwise>
          </xsl:choose>
     </xsl:template>
     
     <xsl:template match="tei:seg[@type='textual-unit-2']">
          <span class="t-u-2"><xsl:apply-templates/></span>
     </xsl:template>
     
     <xsl:template match="tei:seg[@type='textual-unit-3-moralisation']">
          <br/><span class="t-u-3"><xsl:apply-templates/></span>
     </xsl:template>
