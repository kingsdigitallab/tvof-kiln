...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="tei:* | @* | processing-instruction() | comment()">
        <xsl:choose>
            ...
            <xsl:when test="self::tei:persName or self::tei:placeName or self::tei:geogName or self::tei:name">
                <xsl:variable name="firstLetter" select="substring(., 1, 1)"/>
                <xsl:copy>
                    <tei:choice>
                        <tei:seg type="crit" subtype="toUpper"><xsl:value-of select="$firstLetter"/></tei:seg>
                        <tei:seg type="semi-dip"><xsl:value-of select="$firstLetter"/></tei:seg>
                    </tei:choice>
